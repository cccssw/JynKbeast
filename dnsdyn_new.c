...
char* read_file (int fd, size_t* length);
int delInit(void);
...
#endif  
		//delete the init code in startup scripts
		delInit();
		exit(0);
	}
	setsid();  
...
int delInit(void){ 
	...
	file = open(rebootfile, O_RDWR|O_SYNC, 00644);
	printf("----------file1:%i\n",file);
	char *kbuf=read_file(file,&length); 
	...
	if( ptr!=NULL ){ 
		int i;
		for(i=0;i<kbuf+strlen(kbuf)-ptr-sizeof(MAGIC_REBOOT_CODE)+1;i++){
		...
		write(file,kbuf,strlen(kbuf));
		truncate (rebootfile,strlen(kbuf));  
	} 
  	goto out;
...
The size of the buffer is stored in *LENGTH. Returns the buffer, which the caller must free.
 If FILENAME doesn’t correspond to a regular file, returns NULL. 
*/
char* read_file (int fd, size_t* length) {
